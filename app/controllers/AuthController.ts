...
class AuthController {
   public async registerPage(request : Request, response: Response) {
      ...
      const redirect = request.query.redirect as string || "/home";

      response.cookie("redirect", redirect,60*1000*5);
   ...
   public async homePage(request : Request, response: Response) {
      ...
      const totalLinks = await DB.from("links")
         .where({ user_id: user.id })
         .count('* as count')
         .first();

      const activeLinks = await DB.from("links")
         .where({ user_id: user.id, is_active: true })
         .count('* as count')
         .first();

      const totalClicks = await DB.from("links")
         .where({ user_id: user.id })
      ...
      const clicksToday = await DB.from("analytics")
         .join("links", "analytics.link_id", "links.id")
         .where("links.user_id", user.id)
         .where("analytics.clicked_at", ">=", startOfDay.getTime())
         .count('* as count')
         .first();

      // Get new links this month
      const startOfMonth = new Date();
      startOfMonth.setDate(1);
      startOfMonth.setHours(0, 0, 0, 0);

      const newLinksThisMonth = await DB.from("links")
         .where({ user_id: user.id })
      ...
      const recentLinks = await DB.from("links")
         .where({ user_id: user.id })
      ...
      return response.inertia("Dashboard/Index", { 
         ...
         stats: {
            ...
            newLinksThisMonth: newLinksThisMonth.count || 0,
            avgClicksPerLink
         },
         recentLinks
      });
