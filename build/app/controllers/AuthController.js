...
class AuthController {
    ...
    async homePage(request, response) {
        ...
        const totalLinks = await DB_1.default.from("links")
            .where({ user_id: user.id })
            .count('* as count')
            .first();
        const activeLinks = await DB_1.default.from("links")
            .where({ user_id: user.id, is_active: true })
            .count('* as count')
            .first();
        const totalClicks = await DB_1.default.from("links")
            .where({ user_id: user.id })
            .sum('click_count as total')
            .first();
        const startOfDay = new Date();
        startOfDay.setHours(0, 0, 0, 0);
        const clicksToday = await DB_1.default.from("analytics")
        ...
        const newLinksThisMonth = await DB_1.default.from("links")
            .where({ user_id: user.id })
            .where("created_at", ">=", startOfMonth.getTime())
            .count('* as count')
            .first();
        const recentLinks = await DB_1.default.from("links")
            .where({ user_id: user.id })
